...
class IAPoster_Settings_Validator {
	...
	private function validate_field( $value, $setting ) {
		...
		if ( isset( $setting['validator'] ) && is_callable( $setting['validator'] ) ) {
			$custom_val_res = call_user_func( $setting['validator'], $value );
			if ( true !== $custom_val_res ) {
				return $custom_val_res;
			}
		}
