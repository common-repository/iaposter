 ...
 */
class IAPosterPinLib_Transport_Request {
	 ...
	 *
	 * @var IAPosterPinLib_Utils_CurlBuilder
	 */
	private $curlbuilder;
	 ...
	 */
	public function __construct( $curlbuilder ) {
	 ...
	 *
	 * @return IAPosterPinLib_Transport_Response
	 */
	 ...
	 *
	 * @return IAPosterPinLib_Transport_Response
	 * @throws IAPosterPinLib_Exceptions_CurlException
	...
	public function execute( $method, $apiCall, array $parameters = array(), $headers = array() ) {
		...
		if ( $response_data === false && ! $ch->hasErrors() ) {
			throw new IAPosterPinLib_Exceptions_CurlException( "Error: Curl request failed" );
		} else if ( $ch->hasErrors() ) {
			throw new IAPosterPinLib_Exceptions_PinterestException( 'Error: execute() - cURL error: ' . $ch->getErrors(), $ch->getErrorNumber() );
		}

		// Initiate the response
		$response = new IAPosterPinLib_Transport_Response( $response_data, $ch );

		// Check the response code
		if ( $response->getResponseCode() >= 400 ) {
			throw new IAPosterPinLib_Exceptions_PinterestException( 'Pinterest error (code: ' . $response->getResponseCode() . ') with message: ' . $response->getMessage(), $response->getResponseCode() );
		}
